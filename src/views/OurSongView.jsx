...
import { ArrowLeft, Play, Pause, SkipBack, SkipForward, Shuffle, Repeat, Plus, X, Music } from 'lucide-react';
...
const OurSongView = ({ active, onBack, onGlobalMusicPause }) => {
  ...
  return (
    <div id="lyrics-screen" className="screen active">
      ...
      <div className="spotify-container">
        ...
        <div className="spotify-layout">
          ...
          <div className="spotify-player-left">
            <div className={`spotify-album-card ${isPlaying ? 'playing' : ''}`}>
            ...
            <div className="spotify-controls">
              <button
                className={`spotify-ctrl-btn ${isShuffle ? '' : 'secondary'}`}
                title="Shuffle"
                onClick={() => { setIsShuffle(!isShuffle); pickRandomCover(); }}
              >
                <Shuffle className="w-5 h-5" style={{ color: isShuffle ? '#c0392b' : undefined }} />
              </button>
              <button
              ...
              <button
                className={`spotify-ctrl-btn ${isRepeat ? '' : 'secondary'}`}
                title="Repeat"
                onClick={() => setIsRepeat(!isRepeat)}
              >
                <Repeat className="w-5 h-5" style={{ color: isRepeat ? '#c0392b' : undefined }} />
              </button>
            </div>
          </div>
          ...
          <div className="spotify-player-right">
            <div className="spotify-lyrics-scroll" ref={lyricsListRef}>
              {parsedLyrics.length === 0 ? (
                <div className="text-white/40 text-center py-20 italic flex flex-col items-center gap-4">
                  <Music className="w-10 h-10 opacity-30" />
                  <p>No timestamped lyrics uploaded for this song.</p>
                </div>
              ) : (
      ...
      {showUpload && (
        <div className="modal-form-overlay" onClick={() => setShowUpload(false)}>
          <div className="modal-form-content glass-card relative max-w-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleUploadSong} className="flex flex-col gap-4">
              ...
              <div className="form-group">
                ...
                <label className="glass-input flex items-center justify-center gap-2 cursor-pointer hover:border-[#c0392b] transition py-4">
                  <Music className="w-5 h-5 text-[#8a8a93]" />
                  <span className="text-[#8a8a93] text-sm">
                    {audioFile ? audioFile.name : 'Tap to choose MP3'}
                  </span>
                  <input
                    type="file"
