...
import { ArrowLeft, Heart, Plus, X, Sparkles } from 'lucide-react';
...
const ReasonsView = ({ onBack, active }) => {
  ...
  useEffect(() => {
    if (!active || reasons.length === 0) return;
  ...
  return (
    <div id="reasons-screen" className="screen active">
      <div className="container">
        ...
        <div className="tabs-container glass-card mb-8">
          <button
            ...
            onClick={() => { setActiveTab('his'); setVisibleItems(new Set()); }}
          >
            His Reasons
          </button>
          <button
            className={`tab-btn ${activeTab === 'her' ? 'active' : ''}`}
            onClick={() => { setActiveTab('her'); setVisibleItems(new Set()); }}
          >
            Her Reasons
          </button>
        </div>

        <div className="reasons-list-wrapper" ref={listRef}>
          {activeReasons.length === 0 ? (
            <div className="empty-tab-placeholder glass-card">
              ...
              <p className="text-sm mt-2 opacity-60">Add the first one!</p>
            </div>
          ) : (
            <ul className="reasons-list">
              {activeReasons.map((reason, index) => (
                <li
                  key={reason.id}
                  className={`reason-item glass-card ${visibleItems.has(String(index)) ? 'show' : ''}`}
                  style={{ '--item-index': index + 1, transitionDelay: `${index * 0.06}s` }}
                  data-index={index}
                >
      ...
      {showAdd && (
        <div className="modal-form-overlay" onClick={() => setShowAdd(false)}>
